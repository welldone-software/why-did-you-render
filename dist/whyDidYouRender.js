...
function getDisplayName(type) {
  return type.displayName || type.name || type.type && getDisplayName(type.type) || type.render && getDisplayName(type.render) || (lodash.isString(type) ? type : undefined);
}
...
function trackHookChanges(hookName, _ref, hookResult, React, options, ownerDataMap, hooksRef) {
  ...
  if (prevHookResult !== initialHookValue) {
    var notification = getUpdateInfo({
      ...
      displayName: displayName,
      hookName: hookName,
      prevHook: hookPath ? lodash.get(prevHookResult, hookPath) : prevHookResult,
      nextHook: nextHook,
      options: options,
      ownerDataMap: ownerDataMap
    });
...
function whyDidYouRender(React, userOptions) {
  ...
  var hooksSupported = !!React.useState;
